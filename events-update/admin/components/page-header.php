        ...
        <nav class="mb-3 flex items-center gap-1.5 text-sm" aria-label="Breadcrumb">
            <?php foreach ($pageHeaderBreadcrumb as $i => $crumb): ?>
                ...
                <?php if (!empty($crumb['url']) && $i < count($pageHeaderBreadcrumb) - 1): ?>
                    <a href="<?= e($crumb['url']) ?>" class="font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"><?= e($crumb['label']) ?></a>
                <?php else: ?>
                    <span class="font-medium text-gray-800 dark:text-gray-200"><?= e($crumb['label']) ?></span>
                <?php endif; ?>
            <?php endforeach; ?>
        </nav>
    <?php endif; ?>
