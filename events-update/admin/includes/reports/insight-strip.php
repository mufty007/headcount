...
$severityRing = [
    ...
    'info' => 'border-gray-200 bg-gray-50/80 dark:border-gray-600 dark:bg-gray-800/80',
];
?>
<?php if (!empty($insights)): ?>
<div class="mb-8">
    <h3 class="mb-3 text-[10px] font-black uppercase tracking-widest text-gray-400 dark:text-gray-500">Insights</h3>
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        <?php foreach ($insights as $ins): ?>
            ...
            <div class="rounded-2xl border p-4 shadow-card dark:shadow-none <?= e($ring) ?>">
                ...
                <p class="mt-1 text-sm leading-snug text-gray-600 dark:text-gray-300"><?= e($ins['body']) ?></p>
                <?php if (!empty($ins['metric'])): ?>
                    <p class="mt-2 text-xs font-semibold text-brand-700 dark:text-brand-300"><?= e((string) $ins['metric']) ?></p>
                <?php endif; ?>
            </div>
        <?php endforeach; ?>
    </div>
