...
CREATE TABLE IF NOT EXISTS `organizations` (
  ...
  `kiosk_mode` VARCHAR(20) NOT NULL DEFAULT 'split',
...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  `role` ENUM('admin', 'coordinator', 'member', 'presenter') NOT NULL DEFAULT 'member',
  `is_super_admin` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '1=org owner, always full access, cannot be locked out',
  `can_approve_requests` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '1=selected owner: notified and may approve event/program requests',
  `status` ENUM('active', 'inactive', 'deleted') NOT NULL DEFAULT 'active',
  `email_verified_at` DATETIME NULL DEFAULT NULL,
  `last_login_at` TIMESTAMP NULL,
  `pwa_guide_seen_at` DATETIME NULL DEFAULT NULL,
