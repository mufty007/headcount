...
<div class="max-w-6xl mx-auto px-4 py-8" x-data="pd(<?= (int) $id ?>)" x-init="load()">
    ...
    <template x-if="!loading && notFound">
        <div class="bento-card text-center py-16 px-6">
            ...
            <h2 class="text-xl font-bold text-gray-900">Program not found</h2>
            <p class="text-gray-500 mt-2">This program may have been removed or is no longer published.</p>
            <a href="<?= htmlspecialchars($baseUrlPath) ?>/portal/programs.php" class="inline-block mt-6 text-indigo-600 font-semibold hover:underline">Browse programs</a>
        </div>
    </template>
    ...
    <template x-if="!loading && program">
        <div>
                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="bento-card flex items-start gap-4 p-5 hover:border-indigo-200 transition-colors">
                            ...
                            <div>
                                <h3 class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Next session</h3>
                                <p class="font-bold text-gray-900 leading-tight" x-text="formatNextSession(program.next_session) || 'TBA'"></p>
                            </div>
                        </div>
                        <div class="bento-card flex items-start gap-4 p-5 hover:border-indigo-200 transition-colors">
                            <div class="w-12 h-12 rounded-2xl bg-indigo-50 flex items-center justify-center shrink-0">
                            ...
                            <div class="flex-1 min-w-0">
                                <template x-if="program.is_virtual == 1 || program.is_virtual === true">
                                    <div>
                                        <h3 class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Virtual</h3>
                                        <template x-if="program.location && String(program.location).trim()">
                                            <a :href="(String(program.location).match(/^https?:\/\//i) ? program.location : 'https://' + program.location)" target="_blank" rel="noopener noreferrer" class="text-sm text-indigo-600 font-bold hover:underline break-all" x-text="program.location"></a>
                                        </template>
                                        <template x-if="!program.location || !String(program.location).trim()">
                                            <span class="text-gray-500">Link TBA</span>
                                        </template>
                                    </div>
                                </template>
                                <template x-if="!(program.is_virtual == 1 || program.is_virtual === true)">
                                    <div>
                                        ...
                                        <p class="font-bold text-gray-900 leading-tight break-words" x-text="(program.location && String(program.location).trim()) ? program.location : 'TBA'"></p>
                                        <template x-if="program.location && String(program.location).trim() && !(program.is_virtual == 1)">
                                            <a :href="'https://maps.google.com/?q=' + encodeURIComponent(program.location)" target="_blank" class="text-xs text-indigo-600 font-bold hover:underline mt-1 inline-block">View on Maps</a>
                                        </template>
                                    </div>
                                </template>
                            </div>
                    ...
                    <div class="bento-card p-8" x-show="program.presenters && program.presenters.length">
                        <h2 class="text-xl font-bold text-gray-900 mb-6 flex items-center gap-2">
                            <svg width="20" height="20" class="w-5 h-5 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                            Presenters
                        </h2>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <template x-for="(pr, idx) in (program.presenters || [])" :key="idx">
                                <article class="flex gap-4 rounded-2xl border border-gray-100 bg-gray-50/50 p-4">
                                    ...
                                    <template x-if="!pr.image_url">
                                        <div class="w-16 h-16 rounded-xl bg-indigo-100 shrink-0 flex items-center justify-center text-indigo-600 font-bold text-lg" aria-hidden="true" x-text="(pr.display_name && pr.display_name.charAt(0)) ? pr.display_name.charAt(0) : '?'"></div>
                                    </template>
                                    <div class="min-w-0">
                                        <h3 class="font-bold text-gray-900" x-text="pr.display_name || ''"></h3>
                                        <p class="text-sm text-gray-600 mt-0.5" x-show="pr.title" x-text="pr.title"></p>
                                    </div>
                                </article>
                            </template>
                        </div>
                    ...
                    <div class="bento-card p-8">
                        ...
                        <div class="prose max-w-none text-gray-600 leading-relaxed space-y-4" x-html="safeDescriptionHtml()"></div>
                    </div>
                </div>

                <div class="lg:sticky lg:top-24 h-fit">
                    <div class="bento-card overflow-hidden shadow-xl border-indigo-100 p-6 md:p-8 space-y-6">
                        <div class="flex items-center justify-between">
                            <div>
                                <h3 class="text-sm font-bold text-gray-400 uppercase tracking-widest">Entry</h3>
                                <p class="text-3xl font-black text-gray-900" x-text="(program.pricing_type || 'free') === 'free' ? 'Free' : ('$' + (program.price_amount != null ? Number(program.price_amount).toFixed(2) : '0.00'))"></p>
                            </div>
                            <div class="w-14 h-14 rounded-full bg-green-50 flex items-center justify-center text-2xl" x-show="(program.pricing_type || 'free') === 'free'">🎁</div>
                            <div class="w-14 h-14 rounded-full bg-amber-50 flex items-center justify-center text-2xl" x-show="(program.pricing_type || 'free') !== 'free'">💰</div>
                        </div>
                        <div class="h-px bg-gray-100"></div>

                        <div class="rounded-2xl border border-green-100 bg-green-50 p-4 text-center" x-show="reg && reg.status === 'active'">
                            ...
                            <p class="text-green-800 font-bold">You&apos;re registered</p>
                            <a href="<?= htmlspecialchars($baseUrlPath) ?>/portal/my-programs.php" class="text-indigo-600 text-sm font-semibold mt-2 inline-block hover:underline">View My Programs</a>
                        </div>

                        <div class="space-y-4" x-show="!reg || reg.status === 'pending'">
                            <template x-for="q in (program.questions || [])" :key="q.id">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700" x-text="q.question_text + (q.is_required == 1 || q.is_required === true ? ' *' : '')"></label>
                                    <template x-if="(q.question_type || 'short_text') === 'text'">
                                        <textarea class="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" rows="3" x-model="answers[q.id]"></textarea>
                                    </template>
                                    <template x-if="(q.question_type || 'short_text') === 'short_text' || (q.question_type || 'short_text') === 'number'">
                                        <input class="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" :type="(q.question_type || 'short_text') === 'number' ? 'number' : 'text'" x-model="answers[q.id]">
                                    </template>
                                        ...
                                        <div class="mt-2 space-y-2">
                                            <template x-for="opt in (q.options || [])" :key="opt.id">
                                                <label class="flex items-center gap-2 cursor-pointer">
                                                    <input type="radio" class="text-indigo-600 focus:ring-indigo-500" :name="'pq_' + q.id" :value="opt.option_label" x-model="answers[q.id]">
                                                    <span class="text-sm text-gray-800" x-text="opt.option_label"></span>
                                                </label>
                                            </template>
                                        </div>
                                    </template>
                                    <template x-if="(q.question_type || 'short_text') === 'dropdown'">
                                        <select class="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white" x-model="answers[q.id]">
                            ...
                            <div class="rounded-xl border border-gray-200 bg-gray-50 p-3 space-y-2" x-show="program && program.waiver && program.waiver.enabled">
                                <label class="flex items-start gap-2 cursor-pointer">
                                    <input type="checkbox" x-model="waiverAccepted" class="mt-0.5 w-4 h-4 text-indigo-600 rounded border-gray-300 shrink-0">
                                    <span class="text-sm text-gray-700" x-text="program.waiver ? program.waiver.checkbox_label : ''"></span>
                                </label>
                                <button type="button" @click="showWaiverModal = true" class="text-xs font-semibold text-indigo-600 hover:text-indigo-800 underline text-left">Read full waiver</button>
                            </div>
                            <div x-show="(program.pricing_type || 'free') !== 'free'">
                                <label class="block text-sm font-medium text-gray-700">Coupon code (optional)</label>
                                <input type="text" x-model="coupon" class="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm" placeholder="CODE">
                            </div>
            ...
            <div class="lg:hidden fixed bottom-0 left-0 right-0 p-4 bg-white/90 backdrop-blur-xl border-t border-gray-100 z-40" style="margin-bottom: var(--bottom-nav-height, 64px)" x-show="program && (!reg || reg.status === 'pending')">
    ...
    <div x-show="showWaiverModal" x-cloak class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" @keydown.escape.window="showWaiverModal = false">
        <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[85vh] flex flex-col" @click.outside="showWaiverModal = false">
            <div class="p-5 border-b border-gray-100"><h3 class="text-lg font-bold text-gray-900">Liability waiver</h3></div>
            <div class="p-5 overflow-y-auto text-sm text-gray-700 leading-relaxed whitespace-pre-wrap" x-text="program && program.waiver ? program.waiver.full_text : ''"></div>
            <div class="p-4 border-t border-gray-100"><button type="button" @click="showWaiverModal = false" class="w-full py-2.5 bg-indigo-600 text-white rounded-xl font-semibold hover:bg-indigo-700">Close</button></div>
        </div>
    </div>
</div>
...
function pd(id) {
    return {
        ...
        safeDescriptionHtml() {
            ...
            return s || '<p class="text-gray-400 italic">No description provided for this program.</p>';
        },
