...
if (!$service->tablesExist()) {
    jsonResponse(['success' => false, 'message' => 'Event requests are not installed. Run migration 084_event_requests.sql.'], 503);
}
...
$proposalFromInput = static function (array $src): array {
    return [
        ...
        'event_date' => trim((string) ($src['event_date'] ?? '')),
        'start_time' => trim((string) ($src['start_time'] ?? '')),
        'end_time' => trim((string) ($src['end_time'] ?? '')),
...
try {
    ...
    if ($action === 'create' && $method === 'POST') {
        ...
        jsonResponse(['success' => true, 'id' => $id, 'message' => 'Event request submitted.']);
    }
    ...
    if ($action === 'approve' && $method === 'POST') {
        ...
        $id = (int) ($input['id'] ?? 0);
        $eventId = $service->approve($id, $organizationId, $userId, trim((string) ($input['comment'] ?? '')));
        jsonResponse(['success' => true, 'event_id' => $eventId, 'message' => 'Request approved. A draft event was created.']);
    }
    ...
    error_log('event-requests API: ' . $e->getMessage());
    jsonResponse(['success' => false, 'message' => 'Unable to process event request.'], 500);
}
