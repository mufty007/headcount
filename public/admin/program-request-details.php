<?php
/**
 * Program request detail + review actions
 */
...
if (!$service->tablesExist()) {
    ...
    echo 'Program requests are not installed.';
    exit;
}
...
if (!$request) {
    ...
    redirect($adminBase . '/index.php?page=program-requests');
    exit;
}
...
if (isPost()) {
    ...
    try {
        ...
        if ($action === 'send_back' && $canApprove) {
            ...
            $programId = $service->approve($id, $organizationId, $userId, (string) post('comment', ''));
            setFlash('success', 'Request approved. A draft program was created.');
            redirect($adminBase . '/index.php?page=program-edit&id=' . (int) $programId);
            exit;
        } else {
    ...
    redirect($adminBase . '/index.php?page=program-request-details&id=' . $id);
    exit;
}

$comments = $service->commentsFor($id);
$pageTitle = 'Program Request';
$currentPage = 'program-request-details';
require __DIR__ . '/includes/header.php';
$flash = getFlash();
$pending = ($request['status'] ?? '') === ProgramRequestService::STATUS_PENDING;
$needsUpdates = ($request['status'] ?? '') === ProgramRequestService::STATUS_CHANGES_REQUESTED;
$approved = ($request['status'] ?? '') === ProgramRequestService::STATUS_APPROVED;

$fmtDate = !empty($request['starts_on']) ? date('F j, Y', strtotime($request['starts_on'])) : '—';
$fmtTime = '';
if (!empty($request['session_start_time'])) {
    $fmtTime = date('g:i A', strtotime($request['session_start_time']));
    if (!empty($request['session_end_time'])) {
        $fmtTime .= ' – ' . date('g:i A', strtotime($request['session_end_time']));
    }
}
?>
...
<div class="animate-fade-in max-w-4xl mx-auto">
    ...
    if ($needsUpdates && $isOwner): ?>
        <a class="page-header-btn-primary" href="<?= e($adminBase . '/index.php?page=program-request-form&id=' . $id) ?>">Edit and resubmit</a>
    <?php endif;
    if ($approved && !empty($request['program_id'])): ?>
        <a class="page-header-btn-primary" href="<?= e($adminBase . '/index.php?page=program-edit&id=' . (int) $request['program_id']) ?>">Open draft program</a>
    <?php endif;
    ...
    <?php if ($isOwner && in_array($request['status'], [ProgramRequestService::STATUS_PENDING, ProgramRequestService::STATUS_CHANGES_REQUESTED], true)): ?>
