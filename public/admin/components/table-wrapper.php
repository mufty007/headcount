...
<div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-card dark:border-slate-700 dark:bg-slate-800">
    ...
    <?php if ($tableTitle !== '' || $tableActions !== ''): ?>
        <div class="flex items-center justify-between border-b border-gray-100 px-5 py-4 dark:border-slate-700 sm:px-6">
            <?php if ($tableTitle !== ''): ?>
    ...
    <div class="overflow-x-auto custom-scrollbar">
        <table class="w-full text-left">
            <thead>
                <tr class="border-y border-gray-100 bg-gray-50 dark:border-slate-700 dark:bg-slate-900/60">
                    ...
                    <tr>
                        <td colspan="<?= count($tableColumns) ?>" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center gap-2">
                                <svg class="h-10 w-10 text-gray-300 dark:text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path>
                                </svg>
                                <p class="text-sm text-gray-500 dark:text-slate-400"><?= e($tableEmptyMessage) ?></p>
                ...
                <?php else: ?>
                    <?php foreach ($tableRows as $row): ?>
                        <tr class="transition-colors hover:bg-gray-50 dark:hover:bg-slate-700/40">
                            ...
                            ?>
                                <td class="px-5 py-3.5 text-sm text-gray-700 dark:text-slate-300 sm:px-6 <?= e($col['class'] ?? '') ?>">
