...
?>
<div class="admin-filter-card mb-6 rounded-2xl border border-gray-200 bg-white p-5 shadow-card dark:border-slate-700 dark:bg-slate-800 sm:p-6" x-data="{ open: <?= $filterOpen ? 'true' : 'false' ?> }">
    <button type="button" @click="open = !open" class="flex w-full items-center justify-between rounded-lg text-left transition-colors hover:bg-gray-50/80 dark:hover:bg-slate-700/50">
        <span class="text-sm font-semibold text-gray-900 dark:text-white">Filters</span>
        <span class="text-xs text-gray-500 dark:text-slate-400" x-text="open ? 'Hide' : 'Show'"></span>
    </button>
        ...
        <form method="GET" action="<?= e($reportsBaseUrl) ?>" class="space-y-4">
                ...
                <div class="flex flex-wrap gap-3">
                    <?php foreach ($allowedCategories as $cat): ?>
                        <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-slate-300">
                            <input type="checkbox" name="categories[]" value="<?= e($cat) ?>" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 dark:border-slate-500" <?= in_array($cat, $filters->categories, true) ? 'checked' : '' ?>>
                            <?= e($cat) ?>
                        </label>
                    <?php endforeach; ?>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="event_id" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100">
                ...
                <div>
                    ...
                    <input type="search" name="q" value="<?= e($filters->searchQuery) ?>" placeholder="Contains…" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100">
                </div>
            </div>
            ...
            <div>
                ...
                <select name="facility_id" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="program_id" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100">
                ...
                <div>
                    ...
                    <select name="program_category_id" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="min_rsvp_yes" min="0" value="<?= $filters->minRsvpYes !== null ? (int) $filters->minRsvpYes : '' ?>" placeholder="Any" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Min no-show % (tables)</label>
                    <input type="number" name="min_no_show_pct" min="0" max="100" step="0.1" value="<?= $filters->minNoShowPct !== null ? e((string) $filters->minNoShowPct) : '' ?>" placeholder="Any" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100">
                </div>
                <?php endif; ?>
                ...
                <div>
                    ...
                    <select name="revenue_status" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100">
            ...
            <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-slate-300">
                <input type="checkbox" name="compare" value="1" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 dark:border-slate-500" <?= $filters->compare ? 'checked' : '' ?>>
...
<div class="flex flex-wrap gap-2 mb-6 items-center">
    ...
    <?php foreach ($activeChips as $chip): ?>
        <span class="rounded-full border border-indigo-100 bg-indigo-50 px-3 py-1 text-xs font-medium text-indigo-800 dark:border-indigo-800/50 dark:bg-indigo-950/40 dark:text-indigo-200"><?= e($chip) ?></span>
    <?php endforeach; ?>
</div>
<?php endif; ?>
