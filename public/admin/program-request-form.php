<?php
/**
 * Submit or update a program request
 */
...
AuthMiddleware::requireCan('programs.request');
...
if (!$service->tablesExist()) {
    ...
    echo 'Program requests are not installed. Run migration 087_program_requests.sql.';
    exit;
}
...
if ($id > 0 && !$existing) {
    setFlash('error', 'Program request not found.');
    redirect($adminBase ?? '/admin' . '/index.php?page=program-requests');
    exit;
}
if ($existing && (int) $existing['submitted_by'] !== $userId) {
    http_response_code(403);
    echo 'You can only edit your own program requests.';
    exit;
}
...
$form = [
    ...
    'start_time' => !empty($existing['session_start_time']) ? substr((string) $existing['session_start_time'], 0, 5) : '',
    'end_time' => !empty($existing['session_end_time']) ? substr((string) $existing['session_end_time'], 0, 5) : '',
...
if (isPost()) {
    ...
    $form = [
        ...
        'starts_on' => trim((string) post('starts_on', post('event_date', ''))),
        'session_start_time' => trim((string) post('start_time', '')),
        'session_end_time' => trim((string) post('end_time', '')),
        'start_time' => trim((string) post('start_time', '')),
        'end_time' => trim((string) post('end_time', '')),
        'location' => sanitize(post('location', '')),
        'facility_ids' => headcount_event_facility_ids_from_post($db, $organizationId),
    ...
    $errors = ProgramRequestService::validateProposal($form);
    if (empty($errors)) {
        try {
            if ($existing) {
                $service->updateProposal($id, $organizationId, $userId, $form);
                $service->resubmit($id, $organizationId, $userId, trim((string) $form['notes']));
                setFlash('success', 'Your program request was updated and resubmitted for review.');
            } else {
                $id = $service->create($organizationId, $userId, $form);
                setFlash('success', 'Your program request was submitted for review.');
            }
            if (!isset($adminBase)) {
                require_once __DIR__ . '/includes/layout-vars.php';
            }
            redirect($adminBase . '/index.php?page=program-request-details&id=' . (int) $id);
            exit;
        } catch (\Throwable $e) {
            $errors[] = $e->getMessage();
        }
...
$pageTitle = $existing ? 'Update Program Request' : 'Request Program';
$currentPage = 'program-request-form';
require __DIR__ . '/includes/header.php';
?>

<div class="animate-fade-in max-w-3xl mx-auto">
    <?php
    $pageHeaderTitle = $existing ? 'Update program request' : 'Request a program';
    $pageHeaderSubtitle = $existing
        ? 'Update the proposal using the reviewer comments, then resubmit.'
        : 'Describe the program you want to offer. A selected owner will review it before a draft program is created.';
    $pageHeaderBreadcrumb = [
        ['label' => 'Programs', 'url' => $adminBase . '/index.php?page=programs'],
        ['label' => 'Program Requests', 'url' => $adminBase . '/index.php?page=program-requests'],
        ['label' => $existing ? 'Update' : 'New'],
    ];
    require __DIR__ . '/components/page-header.php';
    ?>

    <?php if ($existing && ($existing['status'] ?? '') === ProgramRequestService::STATUS_CHANGES_REQUESTED && !empty($existing['reviewer_comment'])): ?>
    ...
    <form method="post" class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-sm dark:border-gray-800 dark:bg-white/[0.03] space-y-4">
        ...
        <div>
            <label class="form-label" for="title">Program title <span class="text-red-500">*</span></label>
        ...
        <div class="grid gap-4 sm:grid-cols-3">
            <div>
                <label class="form-label" for="starts_on">Start date <span class="text-red-500">*</span></label>
                <input class="ta-input w-full" type="date" id="starts_on" name="starts_on" required value="<?= e($form['starts_on'] ?? $form['event_date'] ?? '') ?>">
            </div>
            <div>
        ...
        $selectedFacilityIds = [];
        if (!empty($existing['facility_ids'])) {
            $dec = json_decode((string) $existing['facility_ids'], true);
            $selectedFacilityIds = is_array($dec) ? array_map('intval', $dec) : [];
        } elseif (!empty($form['facility_ids']) && is_array($form['facility_ids'])) {
            $selectedFacilityIds = $form['facility_ids'];
        }
        ...
        <div>
            ...
            <input class="ta-input w-full" type="text" id="category" name="category" list="program-request-categories" value="<?= e($form['category']) ?>">
            <datalist id="program-request-categories">
        ...
        <div class="flex flex-wrap gap-3 pt-2">
            ...
            <a class="btn-secondary" href="<?= e($adminBase . '/index.php?page=program-requests') ?>">Cancel</a>
        </div>
    </form>
</div>
