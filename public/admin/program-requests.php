...
$statusFilter = get('status', $canApprove ? ProgramRequestService::STATUS_PENDING : 'all');
$filters = [];
if ($statusFilter !== 'all') {
...
$pageTitle = 'Program Requests';
$currentPage = 'program-requests';
require __DIR__ . '/includes/header.php';

$flash = getFlash();
$statusTabs = [
    ProgramRequestService::STATUS_PENDING => 'Pending',
    ProgramRequestService::STATUS_CHANGES_REQUESTED => 'Needs updates',
    ProgramRequestService::STATUS_APPROVED => 'Approved',
    ProgramRequestService::STATUS_DECLINED => 'Declined',
    'all' => 'All',
];
$statusBadge = [
...
<div class="animate-fade-in">
    <?php
    $pageHeaderTitle = 'Program Requests';
    $pageHeaderSubtitle = $canApprove
        ? 'Review staff proposals, send them back for updates, or approve them into a draft program.'
        : 'Submit a program idea for review. You will be notified when it is sent back, approved, or declined.';
    ...
    ob_start();
    if ($canRequest): ?>
        <a href="<?= e($adminBase . '/index.php?page=program-request-form') ?>" class="page-header-btn-primary">Request Program</a>
    <?php endif;
    ...
    <?php if (!$tablesOk): ?>
        <div class="ta-alert ta-alert-warning">Run migration 087_program_requests.sql to enable program requests.<?= $loadError ? ' ' . e($loadError) : '' ?></div>
    <?php else: ?>
        <div class="mb-6 flex flex-wrap gap-2 rounded-2xl border border-gray-200 bg-white p-2 shadow-theme-sm dark:border-gray-800 dark:bg-white/[0.03]">
            <?php foreach ($statusTabs as $st => $label): ?>
                <a href="<?= e($adminBase . '/index.php?page=program-requests&status=' . urlencode($st)) ?>"
            ...
            <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-sm dark:border-gray-800 dark:bg-white/[0.03]">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    <thead class="bg-gray-50 dark:bg-white/[0.02]">
                        <tr>
                            <th class="px-4 py-3 text-left text-theme-xs font-semibold text-gray-500">Program</th>
                        ...
                        <?php foreach ($requests as $row):
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3 text-theme-sm text-gray-700 dark:text-gray-300">
                                    <?= e(date('M j, Y', strtotime($row['starts_on'] ?? $row['event_date'] ?? 'now'))) ?>
                                    <?php if (!empty($row['session_start_time'])): ?>
                                        <div class="text-theme-xs text-gray-500"><?= e(date('g:i A', strtotime($row['session_start_time']))) ?></div>
                                    <?php endif; ?>
                                </td>
                                <?php if ($canApprove): ?>
                                ...
                                <td class="px-4 py-3 text-right">
                                    <a class="text-theme-sm font-semibold text-brand-600 hover:underline" href="<?= e($adminBase . '/index.php?page=program-request-details&id=' . (int) $row['id']) ?>">View</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
