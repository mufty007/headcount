...
/**
 * Transactional email for the program-request workflow.
 */
class ProgramRequestEmailService extends PortalEmailService
{
    ...
    public function notifyApprovers(array $request, array $approvers, bool $resubmitted = false): void
    {
        ...
        $subject = ($resubmitted ? 'Program request resubmitted: ' : 'New program request: ') . ($request['title'] ?? '');
        $body = '
            <h2>' . ($resubmitted ? 'Program request resubmitted' : 'New program request') . '</h2>
            <p>An program request has been ' . htmlspecialchars($verb) . ' and needs your review.</p>
        ...
        foreach ($approvers as $approver) {
            ...
            $svc->sendEmail(
                ...
                $body,
                $orgId,
                ['email_type' => 'program_request_approver_notify', 'user_id' => $approver['id'] ?? null]
            );
        }
    }
    ...
    public function notifySentBack(array $request, string $comment): void
    {
        $this->sendToSubmitter(
            $request,
            'Your program request needs updates',
            '
            <h2>Please update your program request</h2>
            <p>Hello ' . htmlspecialchars($this->submitterFirstName($request)) . ',</p>
            <p>Your program request <strong>' . htmlspecialchars((string) ($request['title'] ?? '')) . '</strong> was sent back. Please make the requested updates and resubmit.</p>
            ' . $this->formatRequestDetails($request) . '
    ...
    public function notifyDeclined(array $request, string $reason): void
    {
        $this->sendToSubmitter(
            $request,
            'Your program request was declined',
            '
            <h2>Program request declined</h2>
            <p>Hello ' . htmlspecialchars($this->submitterFirstName($request)) . ',</p>
            <p>Unfortunately your program request <strong>' . htmlspecialchars((string) ($request['title'] ?? '')) . '</strong> has been declined.</p>
            ' . $this->formatRequestDetails($request) . '
    ...
    {
        $eventId = (int) ($request['program_id'] ?? $request['event_id'] ?? 0);
        $url = $eventId > 0
            ? $this->adminEventEditUrl($eventId)
            : $this->adminRequestUrl((int) $request['organization_id'], (int) $request['id']);
        $this->sendToSubmitter(
            $request,
            'Your program request was approved',
            '
            <h2>Program request approved</h2>
            <p>Hello ' . htmlspecialchars($this->submitterFirstName($request)) . ',</p>
            <p>Your program request <strong>' . htmlspecialchars((string) ($request['title'] ?? '')) . '</strong> has been approved. A draft event was created from your proposal.</p>
            ' . $this->formatRequestDetails($request) . '
    ...
    {
        $date = !empty($request['starts_on']) ? date('F j, Y', strtotime((string) $request['starts_on'])) : (!empty($request['event_date']) ? date('F j, Y', strtotime((string) $request['event_date'])) : '—');
        $time = trim((string) ($request['session_start_time'] ?? $request['start_time'] ?? ''));
        if ($time !== '') {
            $time = date('g:i A', strtotime($time));
        }
        $end = trim((string) ($request['session_end_time'] ?? $request['end_time'] ?? ''));
        if ($end !== '') {
    ...
    {
        return headcount_app_base_url($this->loadAppConfig()) . '/admin/index.php?page=program-request-details&id=' . $requestId;
    }
    ...
    {
        return headcount_app_base_url($this->loadAppConfig()) . '/admin/index.php?page=program-request-form&id=' . (int) $request['id'];
    }

    private function adminEventEditUrl(int $eventId): string
    {
        return headcount_app_base_url($this->loadAppConfig()) . '/admin/index.php?page=program-edit&id=' . $eventId;
    }
